...
const sections = [
  ...
  { id: 'free-chapters', title: "What's included" },
...
export function NavBar() {
  ...
  return (
    <div ref={navBarRef} className="sticky top-0 z-50">
            ...
            <Popover.Panel className="absolute inset-x-0 top-0 bg-white/95 py-3.5 shadow-sm [@supports(backdrop-filter:blur(0))]:bg-white/80 [@supports(backdrop-filter:blur(0))]:backdrop-blur">
              {sections.map((section, sectionIndex) => (
                ...
                >
                  <span
                    aria-hidden="true"
                    className="font-mono text-sm text-altGreen"
                  >
      ...
      <div className="hidden border-t sm:flex sm:h-32 sm:justify-center sm:border-b sm:border-slate-200 sm:bg-white/95 sm:[@supports(backdrop-filter:blur(0))]:bg-white/80 sm:[@supports(backdrop-filter:blur(0))]:backdrop-blur">
        <ol
          role="list"
          ...
          {sections.map((section, sectionIndex) => (
            <li key={section.id} className="flex [counter-increment:section]">
              <Link
                href={`#${section.id}`}
                className={clsx(
                  'flex w-full flex-col items-center justify-center border-b-2 border-t-2 before:mb-2 before:font-mono before:text-sm before:content-[counter(section,decimal-leading-zero)]',
                  sectionIndex === activeIndex
                    ? 'border-altGreen bg-altGreen/50 text-black before:text-black'
