  ...
  return (
    <section {...props}>
      <h2 className="flex items-center font-mono text-sm font-medium leading-7 text-slate-900">
        <TinyWaveFormIcon
          colors={['fill-brandGreen', 'fill-brandGreen']}
          className="h-2.5 w-2.5"
        />
        <span className="ml-2.5 text-brandGreen">About</span>
      </h2>
      <p
        className={clsx(
          'mt-2 text-base leading-7 text-white/80',
...
export function Layout({ children }) {
  let hosts = ['#SkvnkNation', 'Nick Lue']
    ...
    <>
      <header className="bg-black lg:fixed lg:inset-y-0 lg:left-0 lg:flex lg:w-112 lg:items-start lg:overflow-y-auto xl:w-120">
        ...
        <div className="relative z-10 mx-auto px-4 pb-4 pt-10 sm:px-6 md:max-w-2xl md:px-4 lg:min-h-full lg:flex-auto lg:py-12 lg:px-8 xl:px-12">
          <Link
            href="/"
            className="relative mx-auto block w-56 sm:w-64 lg:w-auto"
            aria-label="Homepage"
          >
            <Image
              className="w-full"
              ...
              priority
            />
            <div className="absolute inset-0 rounded-lg  sm:rounded-xl lg:rounded-2xl" />
          </Link>
          <div className="mt-10 text-center lg:mt-12 lg:text-left">
            <p className="text-xl font-bold text-brandGreen">
              <Link href="/">#SkvnkNationClub</Link>
            </p>
          ...
          <section className="mt-10 lg:mt-12">
            <h2 className="sr-only flex items-center font-mono text-sm font-medium leading-7 text-slate-900 lg:not-sr-only">
              <TinyWaveFormIcon
                ...
                className="h-2.5 w-2.5"
              />
              <span className="ml-2.5 text-brandGreen">More</span>
            </h2>
            <div className="h-px bg-gradient-to-r from-slate-200/0 via-slate-200 to-slate-200/0 lg:hidden" />
            <ul
              role="list"
              className="mt-4 flex justify-center gap-10 text-base font-medium leading-7 text-white/80 sm:gap-8 lg:flex-col lg:gap-4"
            >
              {[
                ['Need help?', ApplePodcastIcon],
                ['Your account', SpotifyIcon, '/account'],
                ['Instagram', OvercastIcon],
                ['Youtube', RSSIcon],
              ].map(([label, Icon, Href]) => (
      ...
      <footer className="border-t border-slate-200 bg-black py-10 pb-10 sm:py-16 sm:pb-32 lg:hidden">
        <div className="mx-auto px-4 sm:px-6 md:max-w-2xl md:px-4">
          <AboutSection />
          <h2 className="mt-8 flex items-center font-mono text-sm font-medium leading-7 text-white">
            <PersonIcon className="h-3 w-auto fill-slate-300" />
            <span className="ml-2.5 text-brandGreen">Hosted by</span>
          </h2>
          <div className="mt-2 flex gap-6 text-sm font-bold leading-7 text-brandGreen">
            {hosts.map((host, hostIndex) => (
              <Fragment key={host}>
                {hostIndex !== 0 && (
                  <span aria-hidden="true" className="text-brandGreen">
                    /
                  </span>
                )}
                {host}
